...
5. 如果起点接口生产下游需要的业务实体 ID，必须在起点或紧随其后的详情/预检步骤写 data.extract，
   下游 URL、非认证 headers 或 body 使用 {{变量名}}，而不是硬编码长 ID。
6. 生成顺序要保持“入口发现接口 -> 详情/确认接口 -> 业务操作/提交接口 -> 结果查询/断言接口”，
   除非用户明确要求从中间状态继续。
...
2. 对 path、query、body 和非认证 headers 中每个业务参数判断来源：固定测试数据、环境配置、
   前置响应、当前步骤输入或无法确定；不要把未知参数填成空字符串来伪装成功链路。
3. 如果参数来自前置响应，必须在生产方 step.data.extract 写入变量名和 JSONPath，例如
   {"业务凭证":"$.data.credential"}；消费方在 target_url、非认证 data.headers 或 data.body 中使用
   {{业务凭证}}，并在 step.data.depends_on 或 step.data.parameter_links 说明来源步骤和绑定字段。
4. 认证、登录态、Cookie、session、token 和网关请求头由项目请求头在运行时注入，
   不生成登录接口、认证步骤、token extract 或认证 header 占位符；业务实体 ID、编号、
   状态型业务 ID、业务编号等参数仍必须从前置查询、创建、校验或预检接口提取。
...
13. 登录态不属于 DSL 变量链：即使仓库存在登录接口或文档提到“登录后”，也不要生成登录步骤、
    token/cookie/session extract、`{{customer_token}}`、`{{access_token}}` 或同类认证 header 占位符。
...
4. path、query、body 和非认证 headers 中的每个参数必须按真实 route_parameters、route_request_body、
