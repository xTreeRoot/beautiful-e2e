...
def _builtin_backend_api_agent() -> dict[str, Any]:
    return {
        ...
        "instructions": (
            ...
            "不要跳到后续详情、目标页或提交接口；"
            "接口链路表和测试数据说明只能写入步骤 data，不能伪装成缺 URL 的接口步骤。"
            "再分析接口之间的前置关系、响应提取点、请求参数来源和缺失的上游生产者接口。"
            "认证、登录态、Cookie、session、token 和网关请求头由项目请求头在运行时注入；"
            "生成 DSL 时不要插入登录接口、认证步骤、token extract 或认证 header 占位符。"
            "对于需要传递的业务 ID、业务编号等参数，"
