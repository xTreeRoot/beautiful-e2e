...
- 模块：归纳接口所属模块、入口候选、适用场景、排除场景和证据来源；模块名只使用主路径段，后续路径段作为子域或关联线索，工作区和子仓库重复扫描到同一路由模块时会合并为一个候选模块。
- 关系：归纳接口之间的变量流，例如上游分页或搜索接口产出业务 ID，下游详情、
  首页或提交接口消费该 ID；如果某个接口的后续路径段命中另一个主模块，会生成候选模块关联关系，而不是拆出斜杠模块。
- 覆盖率：仓库摘要会记录扫描文件数、入库文件数、发现接口数和入库接口数；多模块仓库按构建子模块轮询扫描，优先覆盖 Controller、路由和 OpenAPI 文件，摘要上限只影响最终保留数量，不会让路径靠前的子模块独占接口名额。
