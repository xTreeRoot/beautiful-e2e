...
- 运行：`fullstack` 模式会调用 `POST /api/cases/{case_id}/run/fullstack/stream`，直接执行打开页面、填写、点击、断言等浏览器动作，并展示每一步截图；`backend_api` 模式会调用 `POST /api/cases/{case_id}/run/backend-api/stream`，按 `start`、`inference`、`repair`、`request`、`result`、`done` 或 `error` 事件展示真实 HTTP 请求、状态码、耗时和响应预览。单个接口步骤失败后最多尝试 3 次，每次失败响应都会进入运行期 agent 上下文，下一次尝试前只允许补充业务变量或修复请求体，不会改写认证请求头。节点编辑弹窗的“单节点调试”会先把调试草稿写入当前节点 DSL 的 `metadata.node_debug_draft` 并通过 `PUT /api/cases/{case_id}/graph` 入库，再向运行接口传入 `step_id` 和当前节点草稿 `step_override`，只执行当前接口节点；调试草稿可覆盖请求方法、Path、Path 参数、Query 参数、Body 和期望状态。单节点调试缺少变量时仍会直接提示用户手动补齐参数。
...
- 环境切换：项目面板和项目新建/编辑弹窗按环境保存前端/接口基础地址。前端环境和接口环境可分别从基础地址输入框旁的下拉框选择。接口环境还拥有可搜索的请求头键值行，因此生成的接口请求会复用项目界面中输入的精确值。生成后端接口 DSL 时，认证、登录态、Cookie、session、token 和网关请求头统一视为项目请求头能力，后端只把请求头名称传给模型，不传真实值，也不会把登录接口、token 提取或 `{{customer_token}}` 这类认证 header 占位符写进 DSL。保存前还会移除只有 `/api/pb/` 这类网关前缀、或只有登录态/请求头说明的伪接口节点。运行时仍可通过 `BASE_URL`、`API_BASE_URL`、`E2E_ENV` 和 `REQUEST_HEADERS_JSON` 覆盖。
- 项目分析：创建项目时可立即分析，项目面板也提供更新分析操作。重新分析调用 `POST /api/projects/{project_id}/analyze/stream`，按 `start`、`progress`、`project`、`done` 或 `error` 事件展示仓库扫描、扫描覆盖率、认证画像和索引写入进度，并复用流式弹窗显示耗时。多模块仓库会按构建子模块分组轮询扫描，并优先覆盖 Controller、路由和 OpenAPI 文件；`max_routes` 只限制最终摘要大小，不会让路径靠前的子模块独占接口名额。分析结果会把后端路由、前端 DOM 目标、认证画像和扫描上限命中情况存入 `repositories.index_summary`；后续生成会先复用该持久化索引，再扫描实时文件。认证画像会初判登录方式是环境请求头、可执行登录接口、小程序/第三方外部登录，还是未知。项目分析中心的“接口”页签支持按方法、路径、摘要、参数、请求体字段和来源文件搜索，图谱页签会合并工作区与子仓库重复扫描出的同一路由模块，便于把运行失败反馈回真实路由契约。
- 知识图谱归类：模块名只取接口主路径段，后续路径段不会拆成 `A / B` 模块，而是进入子域或跨模块关联线索；如果后续路径段命中另一个主模块，会在链路图中生成候选模块关联关系。
- 知识图谱搜索：项目知识图谱页签支持相似接口搜索，会按路径片段、HTTP 方法、摘要证据、变量产消和 Body 字段给出候选接口，并可一键定位到所属模块。
...
如果单个接口返回非期望状态码，运行器会把本次请求摘要、失败状态码和响应预览加入失败尝试
上下文；下一次请求前先用确定性规则修正常见枚举/选项对象请求体，仍无法处理时再让运行期
agent 基于失败历史返回变量更新或 body 修复。失败重试最多执行到第 3 次尝试。
