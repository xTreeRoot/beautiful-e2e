...
- 环境切换：项目面板和项目新建/编辑弹窗按环境保存前端/接口基础地址。前端环境和接口环境可分别从基础地址输入框旁的下拉框选择。接口环境还拥有可搜索的请求头键值行，因此生成的接口请求会复用项目界面中输入的精确值。生成后端接口 DSL 时，后端只把已配置请求头名称作为认证上下文传给模型，不传真实值；如果环境里已有 `Authorization`、`X-Token`、`satoken`、`Cookie` 或项目自定义登录头，生成器不应再把同名 header 写成 `{{customer_token}}` 这类前置响应变量。运行时仍可通过 `BASE_URL`、`API_BASE_URL`、`E2E_ENV` 和 `REQUEST_HEADERS_JSON` 覆盖。
...
- 项目级 LLM 上下文：后端会通过统一的 `project_llm_context` 把活动环境、脱敏后的认证画像、仓库分析摘要和共享规则传给生成 DSL LLM 与接口运行辅助 LLM。后续新增 LLM 也应复用这个上下文，避免不同模型对登录方式、环境请求头和业务变量来源产生不一致判断。
...
后续步骤可在 `target_url`、`step.data.headers` 或 `step.data.body` 中写 `{{变量}}`。
